...
const OptionsChainView: React.FC = () => {
  ...
  const [selectedOptionType, setSelectedOptionType] = useState<'calls' | 'puts'>('calls');
  ...
  useEffect(() => {
    if (optionsData && optionsData.expirationDates.length > 0) {
      setSelectedDate(optionsData.selectedDate || optionsData.expirationDates[0]);
    }
  }, [optionsData]);
  ...
  const renderSingleTypeOptionRow = (option: OptionsChain, type: 'calls' | 'puts', index: number) => {
    const isSelected = selectedOption?.strike === option.strike && selectedOptionType === type;
    ...
    return (
      ...
      >
        ...
        <TableCell className="text-right py-3">${option.lastPrice.toFixed(2)}</TableCell>
        <TableCell className="text-right py-3">${option.bid.toFixed(2)}</TableCell>
        <TableCell className="text-right py-3">${option.ask.toFixed(2)}</TableCell>
        <TableCell className={cn("text-right py-3", getChangeColor(option.percentChange))}>
          {option.percentChange >= 0 ? '+' : ''}{option.percentChange.toFixed(2)}%
        </TableCell>
        <TableCell className="text-right py-3">{numberWithCommas(option.volume)}</TableCell>
        <TableCell className="text-right py-3">{numberWithCommas(option.openInterest)}</TableCell>
        <TableCell className="text-right py-3">{(option.impliedVolatility * 100).toFixed(1)}%</TableCell>
...
const renderCombinedOptionsTable = () => {
  ...
  return Array.from(strikesMap.entries()).map(([strike, { call, put }], index) => {
    const callSelected = selectedOption?.strike === strike && selectedOptionType === 'calls';
    const putSelected = selectedOption?.strike === strike && selectedOptionType === 'puts';
    ...
    const rowBaseClass = cn(
      ...
      callInTheMoney ? "hover:bg-blue-100/80" : 
      putInTheMoney ? "hover:bg-blue-100/80" : 
    ...
    return (
      ...
      >
        ...
        {[
          ...
          call ? `${call.percentChange >= 0 ? '+' : ''}${call.percentChange.toFixed(2)}%` : "—",
          ...
          <TableCell 
            ...
            className={cn(
              ...
              cellIndex === 0 && callInTheMoney ? "border-l-2 border-l-blue-600" : "",
              // Background color without hover
              callInTheMoney ? "bg-blue-50" : "",
              // Change color for percentage values
              cellIndex === 3 && call ? getChangeColor(call.percentChange) : "",
        ...
        {[
          ...
          put ? `${put.percentChange >= 0 ? '+' : ''}${put.percentChange.toFixed(2)}%` : "—",
          ...
          <TableCell 
            ...
            className={cn(
              ...
              put ? "cursor-pointer" : "",
              // Apply first-cell border without hover effect
              cellIndex === 0 && putInTheMoney ? "border-l-2 border-l-blue-600" : "",
              // Background color without hover
              putInTheMoney ? "bg-blue-50" : "",
              // Change color for percentage values
              cellIndex === 1 && put ? getChangeColor(put.percentChange) : "",
              // Remove individual cell hover effect
              // "hover:bg-transparent"
            )}
          >
  ...
  if (!optionsData || !optionsData.expirationDates || optionsData.expirationDates.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8">
  ...
  const formatDate = (dateStr: string) => {
    ...
    return date.toLocaleDateString('en-US', { timeZone:'America/New_York', year: 'numeric', month: 'short', day: 'numeric' });
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 text-muted-foreground">
            <span>Current Price: ${optionsData.underlyingPrice.toFixed(2)}</span>
            <span>•</span>
            <span>Div Yield: {(optionsData.dividendYield).toFixed(2)}%</span>
            <span>•</span>
            <span>Int Rate: {(optionsData.interestRate * 100).toFixed(2)}%</span>
          </div>
        </div>
      ...
      <div className="flex space-x-2 mb-2">
        <Button 
          ...
          className={cn("rounded-full", viewMode === 'calls' ? "bg-gray-300 hover:bg-gray-400" : "")}
        >
          Calls Only
        </Button>
        <Button 
          ...
          className={cn("rounded-full", viewMode === 'puts' ? "bg-gray-300 hover:bg-gray-400" : "")}
        >
          Puts Only
        </Button>
        <Button 
          ...
          className={cn("rounded-full", viewMode === 'both' ? "bg-gray-300 hover:bg-gray-400" : "")}
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead colSpan={5} className="text-center border-b-2 border-r-0">CALLS</TableHead>
                  <TableHead className="text-center border-b-2 border-x-0 bg-gray-200 bg-opacity-20">STRIKE</TableHead>
                  <TableHead colSpan={5} className="text-center border-b-2 border-l-0">PUTS</TableHead>
                </TableRow>
                <TableRow className="hover:bg-transparent">
                  {/* Call Headers */}
                  ...
                  {/* Strike Column */}
                  <TableHead className="text-center font-semibold border-x">Strike</TableHead>
            ...
            <Card>
              <CardContent className="p-0 overflow-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="w-[100px] font-semibold">Strike</TableHead>
                      <TableHead className="font-semibold text-right">Last</TableHead>
                      ...
                      <TableHead className="font-semibold text-right">Open Int</TableHead>
                          ...
                          <TooltipProvider delayDuration={300}>
                            <Tooltip>
                              ...
                              <TooltipContent side="top">
                                <p>Theoretical option price using binomial model</p>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        </div>
                        ...
                        <div className="flex items-center justify-end">
                          ...
                          <TooltipProvider delayDuration={300}>
                            <Tooltip>
                              ...
                              <TooltipContent side="top">
                                <p>Early exercise premium (American - European)</p>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        </div>
                  ...
                  <TableBody>
                    {optionsData.calls.map((call, idx) => renderSingleTypeOptionRow(call, 'calls', idx))}
                  </TableBody>
                </Table>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-0 overflow-auto">
                <Table>
                  ...
                  <TableBody>
                    {optionsData.puts.map((put, idx) => renderSingleTypeOptionRow(put, 'puts', idx))}
                  </TableBody>
                </Table>
              </CardContent>
            </Card>
...
>
  {selectedOption && (
    <DialogContent className="max-w-7xl w-[98vw] max-h-[90vh] p-0 overflow-hidden">
      <DialogHeader className="sr-only">
        <DialogTitle>
          {selectedOptionType === 'calls' ? 'Call' : 'Put'} Option Details - Strike ${selectedOption.strike.toFixed(2)}
        </DialogTitle>
      </DialogHeader>
...
<div className="w-full md:w-72 p-6 border-r bg-muted/5">
  ...
  <div className={cn(
    ...
    {selectedOptionType === 'calls' ? 'Call Option' : 'Put Option'}
  </div>
  ...
  <div className="space-y-4">
    {/* Strike Price */}
    <div>
      <h3 className="text-sm font-medium text-muted-foreground">Strike Price</h3>
      <p className="text-2xl font-mono font-semibold">${selectedOption.strike.toFixed(2)}</p>
    </div>
    ...
    <div>
      ...
      <p className="text-xs text-muted-foreground">
        {Math.ceil((new Date(selectedDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))} days remaining
      </p>
    </div>
    
    {/* Spot Price */}
    <div>
      <h3 className="text-sm font-medium text-muted-foreground">Current Spot Price</h3>
      <p className="text-xl">${optionsData.underlyingPrice.toFixed(2)}</p>
      <p className={cn(
        "text-xs",
        selectedOptionType === 'calls' 
          ? (optionsData.underlyingPrice > selectedOption.strike ? "text-green-600" : "text-red-600")
          : (optionsData.underlyingPrice < selectedOption.strike ? "text-green-600" : "text-red-600")
      )}>
        {selectedOptionType === 'calls' 
          ? (optionsData.underlyingPrice > selectedOption.strike 
            ? `$${(optionsData.underlyingPrice - selectedOption.strike).toFixed(2)} in-the-money` 
            : `$${(selectedOption.strike - optionsData.underlyingPrice).toFixed(2)} out-of-the-money`)
          : (optionsData.underlyingPrice < selectedOption.strike 
            ? `$${(selectedOption.strike - optionsData.underlyingPrice).toFixed(2)} in-the-money` 
            : `$${(optionsData.underlyingPrice - selectedOption.strike).toFixed(2)} out-of-the-money`)}
      </p>
    </div>
    
    {/* Current Option Price */}
    <div>
      <h3 className="text-sm font-medium text-muted-foreground">Option Price</h3>
      <div className="flex items-baseline gap-2">
        <p className="text-xl">${selectedOption.lastPrice.toFixed(2)}</p>
        <span className={cn(getChangeColor(selectedOption.percentChange), "text-sm")}>
          {selectedOption.percentChange >= 0 ? '+' : ''}{selectedOption.percentChange.toFixed(2)}%
        </span>
      </div>
    </div>
    ...
    <div className="grid grid-cols-2 gap-2">
      ...
      <div>
        <h3 className="text-xs text-muted-foreground">Open Interest</h3>
    ...
    <div className="border-t pt-4">
      ...
      <div className="space-y-2">
        <div className="flex justify-between">
          ...
          <p>{(selectedOption.impliedVolatility * 100).toFixed(1)}%</p>
        </div>
        
        <div className="flex justify-between">
          <h4 className="text-xs text-muted-foreground">Model Price</h4>
          <p>${selectedOption.americanPrice.toFixed(2)}</p>
        </div>
        
        {selectedOption.earlyExerciseValue > 0.01 && (
          <div className="flex justify-between">
            <h4 className="text-xs text-muted-foreground">Early Ex. Premium</h4>
          ...
          <Tabs defaultValue="tree" className="w-full">
            <div className="sticky top-0 bg-white dark:bg-gray-950 p-4 border-b z-10">
              <TabsList className="grid grid-cols-2 w-full">
                ...
                <TabsTrigger value="surface">Volatility Surface</TabsTrigger>
              </TabsList>
            </div>
                ...
                fallback={
                  <div className="flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">
                      The basic option data is still available in the table.
                    </p>
                  </div>
                }
              >
                <BinomialTree 
                  ...
                  preselectedOptionType={selectedOptionType === 'calls' ? 'call' : 'put'} 
                />
              </ErrorBoundary>
            </TabsContent>
            ...
            <TabsContent value="surface" className="mt-0 p-4">
              <ErrorBoundary
                fallback={
                  <div className="flex flex-col items-center justify-center p-8 text-center">
                    <p className="text-amber-600 font-medium mb-2">
                      Unable to generate volatility surface for this option.
                    </p>
                    <p className="text-muted-foreground text-sm">
                      The basic option data is still available in the table.
                    </p>
              ...
              >
                <VolatilitySurfaceChart highlightStrike={selectedOption.strike} />
              </ErrorBoundary>
            </TabsContent>
          </Tabs>
        </div>
