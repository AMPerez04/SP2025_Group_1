...
const DashboardCards: React.FC = () => {
    ...
    const getPriceChangeSinceLastInterval = () => {
        ...
        return { priceChange: priceChange.toFixed(2), changePercent: changePercent.toFixed(2) };
    };
    ...
    const getIcon = (value: number) => {
        if (value > 0) return <TrendingUp className="size-4 text-muted-foreground" />;
        if (value < 0) return <TrendingDown className="size-4 text-muted-foreground" />;
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    {getIcon(priceChangeLastInterval.priceChange)}
                </CardHeader>
                <CardContent>
                    <div className={`text-2xl font-bold ${getColor(priceChangeLastInterval.priceChange)}`}>
                    {getArrow(priceChangeLastInterval.priceChange)}${priceChangeLastInterval.priceChange}
                    </div>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    {getIcon(priceChangeSincePeriod.priceChange)}
                </CardHeader>
                <CardContent>
                    <div className={`text-2xl font-bold ${getColor(priceChangeSincePeriod.priceChange)}`}>
                    {getArrow(priceChangeSincePeriod.priceChange)}${priceChangeSincePeriod.priceChange}
                    </div>
                    <p className="text-xs text-muted-foreground">
